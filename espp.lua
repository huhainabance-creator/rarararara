-- YBA AutoFarm + AutoSell для Delta
...
local Config = { AutoFarm = true, AutoSell = true, AutoPlay = true }
    ...
    while task.wait(3) do
        if Config.AutoFarm and Workspace:FindFirstChild("Item_Spawns") and Workspace.Item_Spawns:FindFirstChild("Items") then
            for _, item in pairs(Workspace.Item_Spawns.Items:GetChildren()) do
                if item:IsA("Model") and item:FindFirstChild("TouchInterest") or item:FindFirstChild("ProximityPrompt") then
                    local hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
                    if hrp and item.PrimaryPart then
